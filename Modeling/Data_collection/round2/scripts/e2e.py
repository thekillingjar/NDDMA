...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="NDDMA2 Round2 1D non-contiguous end-to-end workflow."
    )
    ...
    parser.add_argument("--round1-model", default="")
...
def invoke(script: Path, args: argparse.Namespace, data_dir: str = "",
    ...
    if args.round1_model and script == FIT:
        command.append(f"--round1-model={args.round1_model}")
    if output_dir:
